...
const activateHandler = async (interaction) => {
...
const deactivateHandler = async (interaction) => {
    await interaction.deferReply();
    const config = configs.GUILD_IDS[interaction.guildId]
    try {
        const ledgerManager = new sc.ledger.manager.LedgerManager({ storage: new sc.ledger.storage.WritableGithubStorage({ apiToken: process.env.GITHUB_SECRET, repo: config.repo, branch: config.branch }) })
...
module.exports = [
    [activateHandler, new SlashCommandBuilder()
        .setName("activate")
        .setDescription('Activates you for payouts.')],
    [deactivateHandler, new SlashCommandBuilder()
        .setName("deactive")
